...
plotClusterQC <- function(obj, 
  ...
  ### Age UMAP ###
  umapDF <- data.frame(Embeddings(object=obj, reduction="umap"), obj$age)
  # Randomize cells before plotting
  set.seed(1)
  umapDF <- umapDF[sample(nrow(umapDF)),]

  pdf(paste0(plotDir, sprintf("/age_UMAP_%s.pdf", subgroup)))
...
getConsensusVarFeatures <- function(objs, nfeatures = 3000, blacklist.genes = NULL, buffer = 1000){
  consFeatures <- SelectIntegrationFeatures(objs, nfeatures = nfeatures+buffer) #Buffer to ensure enough features after filtering
  
  # Filter consensus features based on blacklist
  consFeatures <- consFeatures[which(consFeatures %ni% blacklist.genes)]
