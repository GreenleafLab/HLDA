...
wd <- "/oak/stanford/groups/wjg/skim/projects/LDA/1a_atac_preprocess/lda"
pics_dir <- "/oak/stanford/groups/wjg/skim/projects/LDA/resources/gwas/PICS2"
fin_dir <- "/oak/stanford/groups/wjg/skim/projects/LDA/resources/gwas/finucane_finemapping/release1.1"
...
# Peaks do not have unique names by default... so add a name based on chr, start, end
...
pics_ai_gr <- parsePICSfile(paste0(pics_dir, "/PICS2-AIsnplist-8mar19-annotated.csv"))
pics_ai_gr$source <- "pics_autoimmune"

# GWAS catalog PICS (large file and can take some time >30min)
gwas_cat_gr <- parsePICSfile(paste0(pics_dir, "/PICS2-GWAScat-2023-11-10.txt"), check_header=FALSE)
gwas_cat_gr$source <- "pics_GWC"

# Compare to old PICS dataset if there are new disease types of interest added
gwas_cat_gr_old <- parsePICSfile(paste0(pics_dir, "/PICS2-GWAScat-2023-02-03.txt"), check_header=FALSE)
gwas_cat_gr_old$source <- "pics_GWC_old"

old_traits <- unique(gwas_cat_gr_old$disease_trait)
new_traits <- unique(gwas_cat_gr$disease_trait)

addtl_traits <- new_traits[which(new_traits %ni% old_traits)]
length(addtl_traits) #4257 new traits

gwas_cat_gr_added <- gwas_cat_gr[gwas_cat_gr$disease_trait %in% addtl_traits,]

added_traits_count <- gwas_cat_gr_added$disease_trait %>% table() %>% as.data.frame()
colnames(added_traits_count) <- c("disease_trait", "Freq")
source_traits <- data.frame(disease_trait = gwas_cat_gr_added$disease_trait, source = gwas_cat_gr_added$source) %>% unique()

added_traits_count <- left_join(added_traits_count, source_traits, by = "disease_trait")
readr::write_csv(added_traits_count, "/oak/stanford/groups/wjg/skim/projects/LDA/resources/gwas/disease_traits_added.csv")

# looking up linked_SNP in the GWAS catalog set and copying over for autoimmune PICS
...
full_pics_gr <- suppressWarnings(
  c(
    pics_ai_gr, # Pre-computed AI dataset
...
chain <- import.chain("/oak/stanford/groups/wjg/skim/projects/LDA/resources/liftover/hg19ToHg38.over.chain")
