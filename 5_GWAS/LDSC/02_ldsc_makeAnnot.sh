...
conda activate ldsc #make sure that pybedtools is installed on the ldsc environment (https://daler.github.io/pybedtools/main.html)
...
do
    ...
    do
        ...
        then
            ...
            sbatch -p wjg,biochem,sfgf,crabtree,owners -t 00:30:00 --mem=10G --cpus-per-task=1 \
            --job-name=ldsc_annot --output=${logfile} --error=${logfile} \
            --wrap "/oak/stanford/groups/wjg/skim/projects/LDA/scripts/GWAS/LDSC/make_annot_BOR.py \
