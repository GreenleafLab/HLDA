...
then
    ...
    sbatch -p wjg,biochem,sfgf -t 02:00:00 --mem=50G --cpus-per-task=8 \
    --job-name=ctspeaks --output=${logfile} --error=${logfile} \
    --wrap "Rscript /oak/stanford/groups/wjg/skim/projects/LDA/scripts/GWAS/LDSC/get_cell_type_peaks.R ${peakClass} ${archrPath} ${wd}"
else
