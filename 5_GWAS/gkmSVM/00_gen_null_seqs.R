...
outdir <- "/oak/stanford/groups/wjg/skim/projects/LDA/GWAS/gkmSVM"
...
# purposes.)
cell_types <- getFreqs(atac_proj$FineNamedClust)
...
cts_peaks <- lapply(cell_types, function(ct){
  ...
  peaks <- getClusterPeaks(atac_proj, clusterNames=c(ct), peakGR=peaks_gr, originalScore=TRUE, groupBy = "FineNamedClust")
  # Sort peaks by score
...
cts_peaks <- cts_peaks[unlist(lapply(cts_peaks, length)) > use_peaks*0.65]
...
FclustOrder <- FclustOrder[which(FclustOrder %in% cell_types)]

pmat <- jacc_mat

# Add cluster labels
FclustOrder <- FclustOrder[FclustOrder %in% colnames(pmat)]
pmat <- pmat[FclustOrder, FclustOrder]
