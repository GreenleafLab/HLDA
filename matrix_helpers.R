...
fractionXbyY <- function(x, y, add_total=FALSE, xname="x", yname="y", ylab="proportion"){
  ...
  dplyr::summarize(n = n()) %>% ungroup() %>% # summarize (i.e. tally for each group(s))
...
normFractionXbyY <- function(x, y, xname="x", yname="y", ylab="proportion"){
  # Returns a melted dataframe with the proportion of x by y while adjusting for number of total cells in each y
  # e.g. if x is cluster and y is samples, will return the proportion of each sample 
  # making up the total amount of each cluster (i.e. the proportional contribution of each y to each x)
  ###################################
  # x, y: paired vectors
  ...
  rownames(XbyYdf) <- XbyYdf[,1]
  XbyYdf <- XbyYdf[,-1]
  normXbyYdf <- as.data.frame(t(XbyYdf/rowSums(XbyYdf)) / colSums(XbyYdf/rowSums(XbyYdf)))
  normXbyYdf$xname <- rownames(normXbyYdf)
  
  normXbyYdf <- normXbyYdf %>% pivot_longer(!xname, names_to = yname, values_to = ylab)
  colnames(normXbyYdf) <- c(xname, yname, ylab) 
